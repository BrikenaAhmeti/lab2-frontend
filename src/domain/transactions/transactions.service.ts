...
export const TransactionsService = {
  ...
  get: (id: string) => api.core.get<Transaction>(`${BASE}/${id}`).then(r => r.data),

  create: (payload: CreateTransactionDTO) => api.core.post<Transaction>(BASE, payload).then(r => r.data),

  update: (id: string, payload: UpdateTransactionDTO) =>
    api.core.put<Transaction>(`${BASE}/${id}`, payload).then(r => r.data),

  remove: (id: string) => api.core.delete<void>(`${BASE}/${id}`).then(r => r.data),
};
