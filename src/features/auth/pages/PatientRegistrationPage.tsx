...
const registrationSchema = z
  .object({
    ...
    phone: z.string().trim().min(1, 'auth.phoneRequired'),
    dateOfBirth: z.string().trim().min(1, 'auth.dateOfBirthRequired'),
    gender: z.string().trim().min(1, 'auth.genderRequired'),
...
export default function PatientRegistrationPage() {
  ...
  const resendUrl = `/resend-verification?email=${encodeURIComponent(registeredEmail)}`;
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await authApi.register({
        ...
        personalNumber: optionalText(data.personalNumber),
      });
      setRegisteredEmail(data.email);
      setValues({ ...initialValues, email: data.email });
          ...
          {registeredEmail ? (
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Link to="/login">
                  <Button type="button">{t('auth.backToLogin')}</Button>
                </Link>
                <Link to={resendUrl}>
                  <Button type="button" variant="secondary">{t('auth.resendVerification')}</Button>
                </Link>
              </div>
            </div>
          ) : (
